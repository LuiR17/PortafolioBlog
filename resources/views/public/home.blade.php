                    ...
                    <div class="flex flex-col gap-6 md:w-2/2 justify-center items-center text-center mx-auto">
                        <div class="flex flex-col items-center gap-4">
                            ...
                            <h2 class="text-text-secondary text-base font-normal leading-relaxed max-w-lg">
                                Construyo aplicaciones web escalables y experiencias de usuario intuitivas. Apasionado
                                por el código limpio, la arquitectura moderna y la resolución de problemas complejos con
                                soluciones simples.
                            </h2>
                        </div>
                        <div class="flex gap-4 mt-2">
                            ...
                            @else
                                <button disabled
                                    class="flex cursor-pointer items-center justify-center overflow-hidden rounded-lg h-12 px-6 bg-gray-600 text-white text-base font-bold leading-normal tracking-[0.015em] opacity-50">
                                    <span class="material-symbols-outlined mr-2 text-[20px]">download</span>
                    ...
                    @forelse($skills as $category => $categorySkills)
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach ($categorySkills as $skill)
                                        <span
                                            class="px-3 py-1 rounded-md bg-[#282839] text-xs text-text-secondary font-medium">
                                            {{ $skill->name }}
                                        </span>
                                    @endforeach
                                </div>
                        ...
                        @empty
                            <div class="col-span-full text-center py-12">
                                <p class="text-text-secondary">No hay habilidades disponibles aún. ¡Pronto agregaré mis
                                    tecnologías!</p>
                            </div>
                        @endforelse
                    </div>
            ...
            <section class="w-full px-4 sm:px-10 py-10 max-w-[1080px]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @forelse($featuredProjects as $project)
                        <div onclick="window.location.href='{{ route('public.projects.show', $project->slug) }}'"
                            class="group relative flex flex-col overflow-hidden rounded-xl border border-[#282839] bg-card-dark hover:border-primary/50 transition-all hover:shadow-lg hover:shadow-primary/5 cursor-pointer">
                            <div class="w-full aspect-video bg-cover bg-center overflow-hidden"
                                @if ($project->preview_image) style="background-image: url('{{ Storage::url($project->preview_image) }}')">
                                 @else
                                 style="background-image: url('https://via.placeholder.com/400x225/1c1c27/ffffff?text={{ urlencode($project->name) }}')"> @endif
                            ...
                            <div class="flex gap-2 mt-auto">
                                @if ($project->tags)
                                    @foreach ($project->tags->take(3) as $tag)
                                        <span
                                            class="px-2 py-1 bg-[#282839] text-[10px] text-text-secondary uppercase tracking-wider font-bold rounded">{{ $tag->name }}</span>
                                    @endforeach
                                @endif
                            </div>
                            <div class="flex gap-3 pt-2">
                                @if ($project->demo_url)
                                    <a href="{{ $project->demo_url }}" target="_blank" onclick="event.stopPropagation()"
                                        class="flex-1 flex items-center justify-center gap-2 h-9 rounded bg-[#282839] hover:bg-white hover:text-black transition-colors text-white text-sm font-medium">
                                ...
                                @if ($project->repository_url)
                                    <a href="{{ $project->repository_url }}" target="_blank" onclick="event.stopPropagation()"
                                        class="flex-1 flex items-center justify-center gap-2 h-9 rounded border border-[#282839] hover:border-white transition-colors text-white text-sm font-medium">
                ...
                <div class="relative pl-8 border-l border-[#282839] space-y-10">
                    @forelse($education as $edu)
                        <a href="{{ route('public.projects.show', $project->slug) }}">
                            <div class="relative">
                                <div
                                    class="absolute -left-[39px] p-1 bg-background-dark rounded-full border border-[#282839]">
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                    ...
                                    <span
                                        class="text-sm font-mono text-text-secondary px-3 py-1 bg-[#282839] rounded w-fit">
